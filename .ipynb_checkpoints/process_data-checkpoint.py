        ...
        if slice_dir != None:
            ...
            if slice_dir == 0:
                for j in range(image.shape[0]):
                    new_img = image[j,:,:]
                    new_img = ndimage.zoom(new_img, 0.5)
                    assert(new_img.shape == (256, 256))
...
def save_data(tag, split, array):
    ...
    # end original
    np.savez("new_{}_{}.npz".format(tag, split), numpyarray)
...
#     save_data("data", "test", test_data)
    save_data("data", "full_256", cleaned_dataset)
...
#     save_data("label", "test", test_labels)
    save_data("label", "full_256", cleaned_labels)
